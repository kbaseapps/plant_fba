...
#                print_data=True
            ...
            for condition in self.conditions_ids:
                if(condition not in minmax_expscore_dict):
                    minmax_expscore_dict[condition]={'max':-sys.maxsize-1,'min':sys.maxsize}
                ...
                for prt in mdlrxn_obj['modelReactionProteins']:
                    ...
                    for sbnt in prt['modelReactionProteinSubunits']:
                        ...
                        for feature in sbnt['feature_refs']:
                            ...
                            if(ftr_score < minmax_expscore_dict[condition]['min']):
                                minmax_expscore_dict[condition]['min'] = ftr_score

                            if(ftr_score > minmax_expscore_dict[condition]['max']):
                                minmax_expscore_dict[condition]['max'] = ftr_score
                ...
                if(print_data is True):
                    fh2.write(condition+':'+str(reaction_score[0])+'('+reaction_score[1]+')\n')
                ...
                if(len(prots_str_list)>0 and proteins_string != "[]" and proteins_string != "[()]"):
                    ...
                    fh.write('\t'.join([condition,proteins_string,cpt_id,base_rxn,str(reaction_score[0]),reaction_score[1],'\n']))
    ...
    def integrate_abundances_with_metabolism(self):
        ...
        ####################################################################
        (reaction_values_matrix, model_complexes_dict) = self._integrate_abundances(model_obj,
                                                                                    feature_index,
                                                                                    expdata_obj,
                                                                                    condition_index)
